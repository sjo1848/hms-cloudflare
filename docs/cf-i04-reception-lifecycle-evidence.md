...
- Checkout/handoff → `/api/v1/bookings/{id}/check-out` and reception checkout form → `settled` or `pending-approved` policy is required, pending-approved requires a closing reference, plus review/release/handoff confirmations; policy/reference are persisted, `CheckedIn → CheckedOut`, claims release and room becomes `DIRTY` → `npm run test:cf-i04`; browser journey.
...
- `scripts/cf-i04-browser-regression.mjs`: Playwright browser check-in actual-count validation, typed backend `409` error observability and recovery, reassignment and checkout/handoff journey PASS at 375/390/430/768/1024. At 375 the lifecycle task progression and recovery are exercised; every width reaches and interacts with the reception case workspace, queue and selected-case surface. Screenshot persisted at `output/playwright/cf-i04-reception-lifecycle.png`.
