...
| J-06.12 | `housekeepingQueue.ts` semantic booking-status predicate | target `CHECKED_IN` normalizes to source `CheckedIn` for blocked ranking; `CONFIRMED` remains semantically distinct | API/browser fixture: eligible 907 blocked before 901; negative 908 not promoted |
...
- `scripts/cf-i05-browser-regression.playwright.js`: reproducible Playwright journey for source-priority queue-head selection with known `Room 904` HIGH maintenance ahead of numeric `Room 901`, eligible target `CHECKED_IN` departure `Room 907` blocked ahead of 901, negative `CONFIRMED` departure `Room 908`, orphan departure `Room 906` visibility/safety, `Siguiente tarea` mobile focus/open/close, actual focus transition/return at 375px, Start cleaning, Finish cleaning, maintenance open/resolve, validation states, selected-room Clear form isolation and per-width control interaction.
...
- Browser assertions confirmed `document.documentElement.scrollWidth === window.innerWidth` at 375/390/430/768/1024px, API preserved target `CHECKED_IN` and `CONFIRMED` spellings, semantic priority placed blocked eligible `Room 907` before ordinary DIRTY `Room 901` and non-blocked `Room 908` after it, source priority placed known `Room 904` first and `Siguiente tarea` opened 904 at every width, `Room 906` was absent from eligible rooms but present as a blocked orphan departure without invalid mutation controls, mobile focused-task dialog entry/close at 375/390/430, actual focus entry/return at 375, desktop workspace at 768/1024, real local API success for all four domain mutations, client-side blocking of short reason/resolution values, room-A draft isolation/retention and Clear form clearing only room-B.

### Status representation and carry-forward parity debt

The source uses semantic `Confirmed`, `Cancelled`, `CheckedIn` and `CheckedOut` values while the target booking schema serializes `CONFIRMED`, `CANCELLED`, `CHECKED_IN` and `CHECKED_OUT`. Housekeeping predicates normalize these values before ranking; presentation may retain target serialization. The source also excludes `NoShow` departures. The current target schema does not represent `NoShow`; imported NoShow/status migration parity remains carry-forward debt for booking-status/data-migration work (at latest CF-I09), not a CF-I05 lifecycle expansion.
