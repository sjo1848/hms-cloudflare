...
No hard calendar guard is added to check-in/cancellation because source does not impose one. The overrun reassignment guard is an explicit registered target correction (D2), not an accidental parity claim.

## Source-parity pricing on booking update

For covered date/room changes:

`accommodation_total = total_stay_nights × current selected room price_cents`

then existing extra charges are added. Reassignment uses destination current price; extension uses current assigned room price with the new total night count. Existing invoice is reconciled atomically.

Frozen/contracted-rate pricing is outside this wave.
...
| OPEN NON_BLOCKING | escalate | `escalation_note` min 6; `maintenance.report` | OPEN BLOCKING; room consequence follows occupancy |
...
| RESOLVED | escalate/resolve again | none | rejected/idempotent according to command contract; no new success event |

One open maintenance case per room remains the v1 invariant.
...
`OUT_OF_ORDER` remains existing physical state but this wave defines no new transitions into/out of it.

## Availability overlay

Room is excluded from new advance sale if physical status policy disallows sale, overlapping booking inventory exists, overlapping hold exists, or an open maintenance case is `BLOCKING`.

Immediate readiness additionally requires physical `AVAILABLE`. `NON_BLOCKING` is advisory rather than an independent blocker.
...
Canonical API/evidence ownership is `19-api-command-contract-map.md`; authorized source departures are exactly `20-intentional-target-departures.md`. When a command is not permitted by this matrix, default is reject. BUILD cannot invent another route, transition, cutoff or pricing rule around it.