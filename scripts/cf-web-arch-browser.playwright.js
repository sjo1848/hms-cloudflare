(page) => (async () => {
  ...
  await page.getByRole("heading", { name: "Recepción" }).waitFor();
  ...
  await page.getByRole("button", { name: "Abrir navegación" }).click();
  await page.getByRole("dialog", { name: "Navegación móvil" }).getByRole("link", { name: /Habitaciones/ }).click();
  ...
  return { navigationContinuity: "PASS", mobile: true, desktop: true, history: true };
})()