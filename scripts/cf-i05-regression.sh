...
CI=1 "$wrangler" d1 execute HOTEL_DEMO_DB --local -c apps/api/wrangler.jsonc --command "
  ...
  INSERT OR REPLACE INTO rooms (id,room_number,room_type,status,price_cents) VALUES
    ...
    ('room-e','105','STANDARD','DIRTY',15000);
...
node -e "const r=JSON.parse(require('fs').readFileSync('$tmp_dir/response.json')); if(r.map(x=>x.id).sort().join(',')!=='room-a,room-b,room-e') process.exit(1)"
status=$(request "$base/housekeeping/board"); assert_status "$status" 200
node -e "const r=JSON.parse(require('fs').readFileSync('$tmp_dir/response.json')); if(r.rooms.length!==5||!r.rooms.some(x=>x.room_id==='room-d'&&x.room_status==='Maintenance')) process.exit(1)"
...
if CI=1 "$wrangler" d1 execute HOTEL_DEMO_DB --local -c apps/api/wrangler.jsonc --command "BEGIN; UPDATE rooms SET status='CLEANING' WHERE id='room-e' AND status='DIRTY'; INSERT INTO housekeeping_events (id,room_id,event_type,from_status,to_status,actor_subject,request_id,hotel_id,details_json,created_at) VALUES ('bad-transition','room-e','CLEANING_START','AVAILABLE','CLEANING','subject-a','bad-request','hotel-a','{}','2026-01-01'); COMMIT;" >/dev/null 2>&1; then echo "invalid transition unexpectedly committed" >&2; exit 1; fi
CI=1 "$wrangler" d1 execute HOTEL_DEMO_DB --local -c apps/api/wrangler.jsonc --command "SELECT status FROM rooms WHERE id='room-e'; SELECT COUNT(*) AS events FROM housekeeping_events WHERE id='bad-transition';" --json >"$tmp_dir/rollback.json"
