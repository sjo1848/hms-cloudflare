(page) => (async () => {
  ...
  await page.getByRole("button",{name:/Ana Guest/}).click();await page.getByRole("heading",{name:"Ana Guest"}).waitFor();await page.getByText(/Room 101/).waitFor();
  await page.getByRole("button",{name:"New guest"}).click();await page.getByLabel("Full name").fill("Carla Guest");await page.getByLabel("Email").fill("carla@example.test");await page.getByRole("button",{name:"New guest",exact:true}).last().click();await page.getByRole("button",{name:/Carla Guest/}).waitFor();if(await page.getByLabel("Full name").count())throw new Error("guest create form did not close after create");
  await page.getByRole("button",{name:/Upcoming/}).click();await page.getByRole("button",{name:/Bruno Guest/}).waitFor();if(await page.getByRole("button",{name:/Ana Guest/}).count())throw new Error("upcoming guest filter did not narrow the list");await page.getByRole("button",{name:/All/}).click();
