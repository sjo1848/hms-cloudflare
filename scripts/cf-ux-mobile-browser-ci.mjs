...
const historicalPage=async viewport=>{
  const page=await browser.newPage({viewport});
  await page.addInitScript(()=>localStorage.setItem("hms.locale","en"));
  return page;
};
try {
  const p1=await historicalPage({width:1366,height:812});
  const housekeepingResults=await housekeeping(p1);
  const p2=await historicalPage({width:1366,height:812});
  const roomsGuestsResults=await roomsGuests(p2);
  const p3=await historicalPage({width:1366,height:812});
  const adminResults=await admin(p3);
  const p4=await historicalPage({width:390,height:844});
