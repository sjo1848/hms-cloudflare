...
async function executingDatabase() {
  ...
  await db.exec(`
    ...
    CREATE TABLE room_inventory_nights (
      room_id TEXT NOT NULL,
...
async function seedConfirmedBooking(db: D1Database) {
  await db.prepare("INSERT INTO bookings (id, room_id, status, updated_at) VALUES (?1, ?2, 'CONFIRMED', ?3)")
    .bind(bookingId, roomId, "2026-08-30T03:00:00.000Z").run();
  await db.batch([
    db.prepare("INSERT INTO room_inventory_nights (room_id, stay_date, booking_id) VALUES (?1, ?2, ?3)")
      .bind(roomId, "2027-02-10", bookingId),
    db.prepare("INSERT INTO room_inventory_nights (room_id, stay_date, booking_id) VALUES (?1, ?2, ?3)")
      .bind(roomId, "2027-02-11", bookingId),
  ]);
}

async function durableState(db: D1Database) {
  const booking = await db.prepare("SELECT status, updated_at FROM bookings WHERE id = ?1")
    .bind(bookingId).first<{ status: string; updated_at: string }>();
  const inventory = await db.prepare("SELECT COUNT(*) AS count FROM room_inventory_nights WHERE booking_id = ?1")
  ...
  const events = await db.prepare(`SELECT id, booking_id, action, tenant_id, hotel_id, actor_id, session_id, trace_id, created_at
    FROM agent_mutation_events WHERE booking_id = ?1 ORDER BY created_at`)
    .bind(bookingId).all<{
      id: string;
      booking_id: string;
      action: string;
...
describe("D1BookingRepository cancellation attribution on executing D1", () => {
  it("persists exactly one ACP CANCEL event when ACP wins the CONFIRMED transition", async () => {
    const db = await executingDatabase();
  ...
  it("persists zero ACP CANCEL events when an ordinary cancellation wins before the ACP D1 batch", async () => {
    ...
    // Adversarial interleaving: the agent previously observed CONFIRMED, then the
    // ordinary HMS path wins the conditional transition and inventory cleanup
    // before ACP reaches its transactional cancel batch.
    const ordinaryWinnerAt = "2026-08-30T03:29:59.000Z";
    const ordinaryWinner = await db.prepare("UPDATE bookings SET status = 'CANCELLED', updated_at = ?2 WHERE id = ?1 AND status = 'CONFIRMED'")
      .bind(bookingId, ordinaryWinnerAt).run();
    expect(ordinaryWinner.meta.changes).toBe(1);
    await db.prepare("DELETE FROM room_inventory_nights WHERE booking_id = ?1 AND EXISTS (SELECT 1 FROM bookings WHERE id = ?1 AND status = 'CANCELLED')")
      .bind(bookingId).run();
