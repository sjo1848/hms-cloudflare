...
export async function loadNetworkHotelMetrics(db: OperationalDatabase, range: NetworkDateRange, today: string): Promise<NetworkHotelMetrics> {
  const row = await db.prepare(`SELECT
    ...
    .bind(range.start, range.end, today)
    .first<{ total_rooms: number; occupied_rooms: number; active_bookings_count: number; revenue_cents: number }>();
  ...
  const adr = activeBookings > 0 ? Math.trunc(revenue / activeBookings) : 0;
  return {
