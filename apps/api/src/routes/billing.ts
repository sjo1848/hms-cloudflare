...
export function createBillingRoutes(): BillingApp {
  ...
  app.post("/bookings/:id/extra-charges", async c => {
    ...
    try { await db.batch([db.prepare("INSERT INTO extra_charges (id,booking_id,description,amount_cents,category,created_at) VALUES (?1,?2,?3,?4,?5,?6)").bind(crypto.randomUUID(), id, description, amount, category, now), db.prepare("INSERT INTO financial_events (id,event_type,booking_id,actor_subject,request_id,hotel_id,details_json,created_at) VALUES (?1,'EXTRA_CHARGE',?2,?3,?4,?5,?6,?7)").bind(crypto.randomUUID(), id, c.get("identity").subject, c.get("requestId"), c.get("membership").hotelId, JSON.stringify({ amount_cents: amount, category }), now)]); } catch { throw ApiError.conflict("Extra charge could not be recorded atomically"); }
    return c.json({ ok: true, amount_cents: amount }, 201);
  });
  app.get("/bookings/:id/invoice", async c => { requireCap(c, "billing.invoice.read"); return c.json(await invoiceView(c.get("operationalDatabase"), c.req.param("id"))); });
  ...
  app.post("/billing/close-cash", async c => {
    ...
    try {
      await db.batch([db.prepare("INSERT INTO cash_closures (id,actor_subject,total_amount_cents,cash_amount_cents,card_amount_cents,payment_count,counted_cash_amount_cents,cash_difference_cents,opening_time,closing_time,handoff_to,notes,request_id,hotel_id) SELECT ?1,?2,?3,?4,?5,?6,?7,?7-?4,?8,?9,?10,?11,?12,?13 FROM (SELECT COALESCE(SUM(amount_cents),0) total,COALESCE(SUM(CASE WHEN payment_method='CASH' THEN amount_cents ELSE 0 END),0) cash,COALESCE(SUM(CASE WHEN payment_method<>'CASH' THEN amount_cents ELSE 0 END),0) non_cash,COUNT(*) count FROM payment_entries WHERE received_at >= ?8) s WHERE s.total=?14 AND s.cash=?15 AND s.non_cash=?16 AND s.count=?17 AND NOT EXISTS (SELECT 1 FROM cash_closures WHERE opening_time=?8)").bind(crypto.randomUUID(), c.get("identity").subject, expectedTotal, expectedCash, expectedNonCash, expectedCount, counted, opening, closing, handoff, notes, c.get("requestId"), c.get("membership").hotelId, expectedTotal, expectedCash, expectedNonCash, expectedCount)]);
      const winner = await db.prepare("SELECT id FROM cash_closures WHERE request_id = ?1 AND opening_time = ?2").bind(c.get("requestId"), opening).first();
