...
import "./housekeeping-operational.css";
...
export function HousekeepingPage() {
  ...
  const statusCounts = {
    shift: queue.length,
    dirty: queue.filter(room => room.room_status === "Dirty").length,
    cleaning: queue.filter(room => room.room_status === "Cleaning").length,
    available: queue.filter(room => room.room_status === "Available").length,
    maintenance: queue.filter(room => room.room_status === "Maintenance").length,
  };
  const statusFilters = ["shift", "dirty", "cleaning", "available", "maintenance"] as const;
  ...
  return <section className="housekeeping-workspace">
    ...
    <div className="housekeeping-status-strip housekeeping-status-controls" role="group" aria-label={t("housekeeping.filtersAria")}>
      {statusFilters.map(value => <button type="button" key={value} className={filter === value ? "selected" : ""} aria-pressed={filter === value} onClick={() => setFilter(value)}><strong>{statusCounts[value]}</strong><span>{t(value === "shift" ? "housekeeping.filterShift" : value === "dirty" ? "housekeeping.filterDirty" : value === "cleaning" ? "housekeeping.filterCleaning" : value === "available" ? "housekeeping.filterAvailable" : "housekeeping.filterMaintenance")}</span></button>)}
