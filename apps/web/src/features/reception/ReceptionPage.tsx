...
function Bookings() {
  ...
  return <section className="reception-workspace">
    <div className="workspace-heading reception-workspace-heading">
      ...
      <div className="reception-heading-actions"><span className="case-count">{t("reception.queueSummary", { attention: counts.attention, all: counts.all })}</span><button type="button" className="secondary-button reception-create-trigger" onClick={() => setShowCreate(current => !current)}>{showCreate ? t("common.close") : t("reception.createBooking")}</button></div>
    </div>
    ...
    <div className="case-layout reception-case-layout">
      ...
      {selected ? <article className="case-panel">
        ...
        {selected.status === "Confirmed" ? <form onSubmit={saveEdit} aria-label={t("reception.editAria")}><h4>{t("reception.stayDetails")}</h4><label>{t("common.guest")} <select aria-label={t("reception.editGuest")} value={editForm.guest_id} onChange={e => setEditForm({ ...editForm, guest_id: e.target.value })} required>{guests.map(guest => <option key={guest.id} value={guest.id}>{guest.full_name}</option>)}</select></label><label>{t("common.room")} <select aria-label={t("reception.editRoom")} value={editForm.room_id} onChange={e => setEditForm({ ...editForm, room_id: e.target.value })} required><option value="">{t("reception.selectRoomDates")}</option>{editAvailableRooms.map(room => <option key={room.id} value={room.id}>{room.room_number} · {room.room_type}</option>)}</select></label><label>{t("reception.checkIn")} <input aria-label={t("reception.editCheckIn")} type="date" value={editForm.check_in} onChange={e => setEditForm({ ...editForm, check_in: e.target.value })} required /></label><label>{t("reception.checkOut")} <input aria-label={t("reception.editCheckOut")} type="date" value={editForm.check_out} onChange={e => setEditForm({ ...editForm, check_out: e.target.value })} required /></label><label>{t("common.notes")} <input aria-label={t("reception.editNotes")} value={editForm.notes} onChange={e => setEditForm({ ...editForm, notes: e.target.value })} placeholder={t("reception.notesOptional")} /></label><button>{t("reception.saveChanges")}</button><button type="button" onClick={() => void cancelBooking()}>{t("reception.cancelBooking")}</button><button type="button" onClick={closeCase}>{t("reception.closeCase")}</button></form> : <div className="locked-stay-details"><h4>{t("reception.stayDetails")}</h4><p className="muted">{t("reception.assignmentLocked")}</p></div>}
        {selected.status === "Confirmed" && <form onSubmit={checkIn} aria-label={t("reception.checkInAria")}><h4>{t("reception.nextCheckIn")}</h4><div className="step-progress" aria-label={t("reception.checkInProgress")}>{checkInStepKeys.map((step, index) => <span className={index === mobileStep ? "current" : index < mobileStep ? "complete" : ""} key={step}>{index + 1}. {t(step)}</span>)}</div>{(window.innerWidth >= 768 || mobileStep === 0) && <><p className="step-title">{t(checkInStepKeys[0])}</p><label>{t("reception.finalGuestCount")} <input name="check_in_guests_count" type="number" min="1" max="100" value={checkInData.count} onChange={e => setCheckInData({ ...checkInData, count: e.target.value })} required /></label><label><input type="checkbox" name="document" checked={checkInData.document} onChange={e => setCheckInData({ ...checkInData, document: e.target.checked })} required />{t("reception.documentVerified")}</label></>}{(window.innerWidth >= 768 || mobileStep === 1) && <><p className="step-title">{t(checkInStepKeys[1])}</p><label><input type="checkbox" name="contact" checked={checkInData.contact} onChange={e => setCheckInData({ ...checkInData, contact: e.target.checked })} required />{t("reception.contactConfirmed")}</label><label><input type="checkbox" name="stay" checked={checkInData.stay} onChange={e => setCheckInData({ ...checkInData, stay: e.target.checked })} required />{t("reception.stayConfirmed")}</label></>}{(window.innerWidth >= 768 || mobileStep === 2) && <><p className="step-title">{t(checkInStepKeys[2])}</p><p className="muted">{t("reception.roomAssigned", { room: selected.room_number })}</p></>}{(window.innerWidth >= 768 || mobileStep === 3) && <><p className="step-title">{t(checkInStepKeys[3])}</p><p className="muted">{t("reception.reviewCheckIn")}</p></>}{window.innerWidth < 768 && <div className="step-actions">{mobileStep > 0 && <button type="button" onClick={() => setCheckInStep(mobileStep - 1)}>{t("reception.back")}</button>}<button>{mobileStep < checkInStepKeys.length - 1 ? t("reception.nextStep") : t("reception.completeCheckIn")}</button></div>}{window.innerWidth >= 768 && <button>{t("reception.completeCheckIn")}</button>}</form>}
        {selected.status === "CheckedIn" && <><form onSubmit={reassign} aria-label={t("reception.reassignAria")}><h4>{t("reception.nextReassign")}</h4><select name="room_id" required><option value="">{t("reception.selectDestination")}</option>{rooms.filter(room => room.id !== selected.room_id && room.status === "Available").map(room => <option key={room.id} value={room.id}>{room.room_number}</option>)}</select><button>{t("reception.reassignRoom")}</button></form><form onSubmit={checkout} aria-label={t("reception.checkoutAria")}><h4>{t("reception.nextCheckout")}</h4><label>{t("reception.paymentPolicy")} <select name="policy" required><option value="settled">{t("reception.settled")}</option><option value="pending-approved">{t("reception.pendingApproved")}</option></select></label><label>{t("reception.closingReference")} <input name="reference" minLength={6} placeholder={t("reception.referenceHint")} /></label>{([["charges", "reception.chargesReviewed"], ["release", "reception.roomReleaseConfirmed"], ["handoff", "reception.housekeepingHandoffConfirmed"]] as const satisfies ReadonlyArray<readonly [string, MessageKey]>).map(([name, label]) => <label key={name}><input type="checkbox" name={name} required />{t(label)}</label>)}<button>{t("reception.completeCheckout")}</button></form></>}
