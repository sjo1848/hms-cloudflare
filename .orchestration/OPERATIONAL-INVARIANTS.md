...
- **LATE-001:** late arrival is context on CONFIRMED, not lifecycle. Canonical booking PATCH/front_desk metadata requires future ETA inside `[check_in,check_out)` by hotel-local date, note min 6/max 250, actor/time/audit; no room/inventory/total/invoice mutation.
...
- **EVIDENCE-001:** backend enforces terminal reason min 6, reassignment reason min 6, maintenance escalation/resolution note min 6, late-arrival note min 6.
- **PRICE-001:** source-covered room/date changes use total stay nights × current selected-room price + extras.
...
- **FRONTDESK-READ-001:** `GET /api/v1/front-desk/board` requires `bookings.read`; admin/ops/receptionist allowed, housekeeping/saas_admin denied.
- **API-001:** `19-api-command-contract-map.md` owns routes/payloads/capabilities; no shadow/direct-status shortcut. Late arrival is the defined confirmed-booking metadata use of PATCH.
