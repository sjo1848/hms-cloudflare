...
Codex MUST read and apply `.orchestration/MODEL-REASONING-POLICY.md` when assigning runtime contexts. **Model family, reasoning allocation and role separation are all admission requirements.**

For work already in progress when that policy is introduced/revised, do not restart an already-completed lane solely to retrofit it. Every new lane, reopened lane or REWORK started afterward must use the current policy.
...
Default runtime assignment: `Luna LOW`. Escalate to `Luna MEDIUM` only when cross-lane architecture/risk complexity warrants it.
...
Default runtime assignment: `Luna LOW` for ordinary bounded implementation; `Luna MEDIUM` is required for migration, money, tenant/security, concurrency/data-integrity or equivalent high-risk lanes.
...
Default runtime assignment: `Luna MEDIUM`.
...
`plan → decompose lanes → assign model family + reasoning tier → implement → lane self-tests → Internal QA/Critic → automatic repair → re-test → Integration Review → automatic repair if needed → full regression → evidence claim audit → Pre-Critic Gate → artifact A → boundary B → External Independent Critic`
...
Higher reasoning or a more expensive model family does not waive independence. An Implementer still cannot self-approve.
...
2. **Runtime assignment receipt** — each role/context, lane, actual model family, reasoning tier or truthful runtime equivalent, and any escalation rationale.
...
## Model / reasoning escalation policy

The binding defaults come from `.orchestration/MODEL-REASONING-POLICY.md`:

- Orchestrator: `Luna LOW` by default; `Luna MEDIUM` only when needed;
- ordinary Implementer: `Luna LOW`;
- migration/money/tenant/security/concurrency/data-integrity Implementer: `Luna MEDIUM`;
- Internal QA/Critic: `Luna MEDIUM`;
- Integration Reviewer: `Luna MEDIUM`;
- final high-risk security/readiness review: `Luna HIGH` where contract-relevant;
- `Terra`: bounded escalation only after demonstrated Luna insufficiency;
- `Sol`: prohibited by default; bounded last resort only after Luna and Terra fail on an unresolved P0/P1 issue.

Escalate reasoning inside Luna before changing model family. Terra/Sol usage must be narrow and recorded in the internal review receipt.
...
Artifact A may be published only after the Internal QA/Critic and Integration Reviewer receipts are closed, the **actual model-family + reasoning assignment receipt** is truthful, and the binding Pre-Critic Gate passes. Boundary B then stops execution for External Independent Critic.
