...
| INV-ENUM-001 | APPLIES | PASS | `normalizeBookingStatus` maps `CONFIRMED/CANCELLED/CHECKED_IN/CHECKED_OUT` to source semantics; browser API fixture proves target `CHECKED_IN` eligible Room 907 receives blocked rank/order while target `CONFIRMED` Room 908 does not; UI/API preserve serialized values | business predicates use semantic status, not mismatched storage spelling |
| INV-PARITY-001 | APPLIES | PASS | `docs/cf-i05-housekeeping-maintenance-parity.md`; source queue algorithm mapping; D1/API positive and negative tests; browser fixtures with HIGH maintenance 904, eligible CHECKED_IN departure 907, negative CONFIRMED departure 908 and orphan 906 | source operational ranking and synthetic departure ownership/context are retained |
| INV-ORDER-001 | APPLIES | PASS | `buildHousekeepingQueue` ports maintenance priority, turnover, blocked/status ranks and numeric tie-break; browser asserts known source head `Room 904`, eligible semantic blocked ordering `907 < 901 < 908`, `Siguiente tarea` opens 904, and orphan 906 is visible/safe | expected identities are independent of target-first-button derivation |
...
| INV-STATE-001 | APPLIES | PASS | publication protocol creates substantive artifact commit A followed by orchestration-only boundary commit B; B records exact A, `external_review.required=true` and `resume_authorized=false`; no product code changes in B | non-circular canonical closure; unrelated `install.sh` remains outside artifact |
...
| queue ranking and focused workspace parity | `scripts/cf-i05-browser-regression.playwright.js`, source parity matrix; known 904-vs-901 priority conflict, semantic 907/901/908 ordering, `Siguiente tarea` identity, mobile dialog open/close and queue return assertions | integrated browser + source |
...
| enum representation parity | same browser harness asserts API emits `CHECKED_IN`/`CONFIRMED` and queue behavior follows semantic mapping; eligible 907 is blocked, negative 908 is not promoted to blocked rank | integrated API + browser |
...
- [x] Artifact commit A and the orchestration-only publication-boundary commit B will converge canonical state on exact A before exit.
