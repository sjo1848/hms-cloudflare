...
Artifact candidate: `PENDING-CF-I05-REWORK-2`
...
| INV-UX-001 | APPLIES | PASS | integrated browser script executes queue → selected room → focused workspace; `Siguiente tarea` and queue selection open a mobile dialog; `Cerrar tarea` returns to queue; lifecycle and maintenance controls execute | active route preserves source-equivalent focused-task semantics |
| INV-RESP-001 | APPLIES | PASS | `npm run test:cf-i05-browser`; executable journey at 375/390/430 asserts mobile dialog open/close, 768/1024 assert desktop workspace, all widths assert material queue/control reachability and no overflow | every contracted width exercises the relevant interaction model |
| INV-EVID-001 | APPLIES | PASS | this file and parity doc map ABA, mobile focus/close, draft isolation/retention/reset, API/D1 and screenshot claims to named executable assertions | no claim exceeds committed harness proof |
...
| INV-STATE-001 | APPLIES | PASS | final publication will persist the exact REWORK-2 artifact in `.orchestration/STATE.md` and `.orchestration/STATUS.json`; unrelated `install.sh` remains outside artifact | canonical state is updated together with the immutable publication |
...
| per-room draft isolation/reset | same browser harness enters room 903 draft, switches to 905, verifies empty, returns to 903, verifies retained, then successful action clears intended draft | browser |
