...
| INV-RESP-001 | PASS | Playwright at 375/390/430/768/1024 explicitly asserts create success, visible duplicate error, role commit after refetch, deactivate confirmation/focus return, and Network plan mutation; valid housekeeping membership denial is separately asserted; no horizontal overflow. |
| INV-EVID-001 | PASS | This file, Pre-Critic Gate, source matrix, executable runners and committed browser screenshot match the executed assertions; no unexecuted browser claim is presented. |
...
| INV-CF-I07-002 | PASS | Focal runner repeats same role and same plan, asserts 409, and durably asserts exactly one `HOTEL_PLAN_CHANGE` for the successful transition with no additional event. |
...
| INV-CF-I07-004 | PASS | Focal runner recursively captures its Worker tree, waits/polls termination before PASS, and post-run process inspection found no owned Worker; inherited CF-I03 cleanup was hardened and also ended with no Worker. |
...
- Fresh `scripts/cf-i07-browser-regression.sh`: PASS; every contracted width asserts visible success/error, role persistence and deactivate confirmation/focus, while housekeeping membership is positively established before capability denial.
